/**
 * App.jsx — Study Tracker for Noobs  ·  Zero-Flicker Edition
 *
 * STATE ISOLATION ARCHITECTURE:
 * ─────────────────────────────────────────────────────────────────────────────
 *  useTimerState()    → owns timerSec/running/phase/swTime — ticks here ONLY
 *  useGameState()     → owns xp/level/streak/sessions
 *  useSyllabusState() → owns exam/grade/chapters/selSub
 *  useTaskState()     → owns tasks list
 *  useUIState()       → owns page/sidebarOpen/modals/theme
 *
 *  When the Pomodoro ticks, ONLY useTimerState's slice updates.
 *  React sees: timerSec changed. It re-renders exactly:
 *    • TimerDisplay (reads timerSec via TimerCtx)
 *    • MiniTimer    (reads timerSec via TimerCtx)
 *  Sidebar, Header badges, TaskList, SyllabusPanel → zero re-renders per tick.
 *
 * SOUND ENGINE:
 *  useAudio()  — no external library needed; uses Web Audio API + HTMLAudio
 *    playClick()    → soft UI tap (150 ms sine burst)
 *    playComplete() → task done chime (CDN mp3)
 *    playAlarm()    → Pomodoro zero bell (CDN mp3)
 *
 * ELECTRIC BUTTON:
 *  CSS `will-change: transform` on .zap-ring prevents GPU layer promotion
 *  during parent re-renders; rotation transform is compositor-only.
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
const XP_TASK  = 100;
const XP_POMO  = 100;
const XP_LV    = 500;   // XP required per level
...
// Shared animation presets — defined once, never re-created
...
// ═════════════════════════════════════════════════════════════════════════════
// § 3  SOUND ENGINE  (Web Audio API — no external package needed)
// ═════════════════════════════════════════════════════════════════════════════
...
function useAudio() {
  ...
  // Preload mp3 files lazily on first use
  ...
  // Soft synthetic click via Web Audio API (no CDN needed, ~2 ms)
  ...
  const playComplete = useCallback(() => {
    if (muted.current) return;
    try { const a = getAudio("complete"); a.currentTime = 0; a.play(); } catch {}
  }, [getAudio]);

  const playAlarm = useCallback(() => {
    if (muted.current) return;
    try { const a = getAudio("alarm"); a.currentTime = 0; a.play(); } catch {}
  }, [getAudio]);

  const toggleMute = useCallback(() => { muted.current = !muted.current; return muted.current; }, []);

  return { playClick, playComplete, playAlarm, toggleMute, mutedRef: muted };
}

// ═════════════════════════════════════════════════════════════════════════════
// § 4  STATE SLICES  (custom hooks — each slice is independent)
// ═════════════════════════════════════════════════════════════════════════════
...
function useUIState() {
  ...
  return {
    ...
    showExam, setShowExam,
    showTheme, setShowTheme,
    themeId, setThemeId, th,
    muted, setMuted,
    obStep, setObStep,
  };
}

// ── 4b. TIMER STATE — the ONLY hook that updates every second ─────────────────
function useTimerState({ onPomoComplete, onXpEarned, playAlarm }) {
  ...
  // Stable mutable bag — read inside setInterval without stale closures
  const live   = useRef({});
  live.current = { phase, pomoDur, brkDur, topic };

  // Callback refs so setInterval never captures stale function pointers
  const onCompleteRef = useRef(onPomoComplete);
  const onXpRef       = useRef(onXpEarned);
  const alarmRef      = useRef(playAlarm);
  useEffect(() => { onCompleteRef.current = onPomoComplete; }, [onPomoComplete]);
  useEffect(() => { onXpRef.current       = onXpEarned; },    [onXpEarned]);
  useEffect(() => { alarmRef.current      = playAlarm; },     [playAlarm]);

  // Pomodoro interval — registered only when running/mode change, NOT every tick
  useEffect(() => {
    ...
    ivRef.current = setInterval(() => {
      setTimerSec(s => {
        ...
        if (ph === "work") {
          onXpRef.current?.(XP_POMO);
          onCompleteRef.current?.({ subject: tp || "General Study", dur: pd, time: new Date().toLocaleTimeString() });
          setPhase("break");
          return bd * 60;
        } else {
          setPhase("work");
  ...
  }, [running, timerMode]);  // ← accent/topic/phase intentionally NOT in deps
  ...
  // Sync pomoDur → timerSec when not running
  useEffect(() => { if (!running) setTimerSec(pomoDur * 60); }, [pomoDur, running]);

  // Persist on pause only (not every second)
  useEffect(() => {
    if (!running && !swRun) {
      LS.w("st_timer", { timerMode, pomoDur, brkDur, phase, timerSec, swTime });
    }
  ...
  return {
    timerMode, setTimerMode, pomoDur, setPomoDur, brkDur, setBrkDur,
    timerSec, running, setRunning, phase, swTime, swRun, setSwRun,
    topic, setTopic, resetTimer, switchMode,
  };
}

// ── 4c. GAME STATE — XP, levels, streaks, sessions ───────────────────────────
...
function useSyllabusState() {
  ...
  const switchExam = useCallback((k) => {
    setExamRaw(k); setChapters({}); setSelSub(null);
  }, []);

  const markChapterDone = useCallback((sub, ch) => {
  ...
  return {
    exam, grade, setGrade, chapters, selSub, setSelSub,
    switchExam, markChapterDone, subs, totalCh, doneCh, pct,
  };
}
...
function useTaskState() {
  ...
  useEffect(() => {
    clearTimeout(debRef.current);
    debRef.current = setTimeout(() => LS.w("st_tasks", tasks), 10_000);
    return () => clearTimeout(debRef.current);
  }, [tasks]);

  const addTask = useCallback((text) => {
    setTasks(p => [...p, { id: Date.now(), text, done: false }]);
  }, []);
  ...
  // toggleTask returns the toggled task so callers can react (XP, streak, chap)
...
// ═════════════════════════════════════════════════════════════════════════════
// § 5  CONTEXT  (TimerCtx — only timer slice is broadcast)
// ═════════════════════════════════════════════════════════════════════════════

const TimerCtx = createContext(null);

// ═════════════════════════════════════════════════════════════════════════════
// § 6  PURE PRESENTATIONAL COMPONENTS  (module-scope memo)
// ═════════════════════════════════════════════════════════════════════════════
  ...
  return (
    <div className="flex gap-2">
      <input ref={ref} value={v}
        ...
        className={`flex-1 rounded-xl px-4 text-sm border outline-none transition-colors duration-200 ${light ? "bg-slate-100 border-slate-300 text-slate-900 placeholder-slate-400" : "bg-zinc-800 border-zinc-700 text-zinc-100 placeholder-zinc-500"}`}
      />
...
// ── Mini Timer (header badge — reads context, not props) ──────────────────────
// Only re-renders when TimerCtx value changes (every second while running)
// This component is isolated from the rest of the header by context subscription
const MiniTimer = memo(function MiniTimer({ accent }) {
  const t = useContext(TimerCtx);
  if (!t) return null;
  const active = t.running || t.swRun;
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
      ...
      style={{ background: `${accent}1c`, border: `1px solid ${accent}44`, color: accent, fontFamily: "DM Mono,monospace" }}>
      {icon}
      <span>{fmt(secs)}</span>
...
// ── Sidebar ────────────────────────────────────────────────────────────────────
// Props: th, page, setPage, open, setOpen, accent, xp, level
// ← NEVER includes timer state, so sidebar is immune to per-second re-renders
const Sidebar = memo(function Sidebar({ th, page, setPage, open, setOpen, accent, xp, level, playClick }) {
  ...
  return (
    <motion.aside initial={false} animate={{ width: open ? 220 : 62 }}
      ...
      {/* ⚡ Electric Home — will-change:transform applied via CSS .zap-ring */}
...
// ── Header ────────────────────────────────────────────────────────────────────
// Does NOT receive timer values in props — only reads them via TimerCtx (MiniTimer)
// So header badges never re-render from timer ticks
const Header = memo(function Header({
  th, page, accent, streak, xp, meta,
  showTheme, setShowTheme, themeId, setThemeId,
  setShowExam, switchExam, exam, muted, onToggleMute, playClick,
}) {
  ...
  return (
    <header className="st-topbar glass sticky top-0 z-50 border-b px-4 md:px-6 py-3 flex items-center justify-between">
      ...
      <div className="flex items-center gap-2 relative">
        {/* MiniTimer subscribes to TimerCtx; header itself never re-renders per tick */}
        ...
        {/* Mute toggle */}
        <motion.button {...TAP} onClick={onToggleMute}
...
// ═════════════════════════════════════════════════════════════════════════════
// § 7  ROOT APP  (orchestrates hooks; pages are conditionally rendered, never
//                unmounted, so timer outlives navigation)
// ═════════════════════════════════════════════════════════════════════════════
...
export default function App() {
  ...
  // Stable callbacks from game slice (passed into timer as refs)
  const handlePomoComplete = useCallback((info) => {
    game.addSession(info);
    // XP already awarded via onXpEarned below; just confetti
    burst(accent);
  }, [game.addSession]); // eslint-disable-line

  const handleXpEarned = useCallback((amount) => {
    game.awardXp(amount, accent);
  ...
  // ── Chapter click: adds task only, no XP/completion ───────────────────────
  const handleChapterClick = useCallback((sub, ch) => {
    audio.playClick();
    taskS.addChapterTask(sub, ch);
  }, [taskS.addChapterTask, audio.playClick]);

  // ── Context value — only timer slice; updated every second while running ──
  // Sidebar/Header are NOT subscribed, so they don't re-render per tick
        ...
        <AnimatePresence mode="wait">
          {ui.obStep === 0 ? (
            ...
            <motion.div key="s1" initial={{ opacity: 0, y: 36 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -36 }} className="text-center w-full max-w-sm">
              ...
              <button onClick={() => ui.setObStep(0)} className="mt-5 text-zinc-600 text-sm hover:text-zinc-400 transition-colors"
  ...
  // ═════════════════════════════════════════════════════════════════════════
  // PAGE RENDERERS  (inner functions close over stable hooks; defined here
  // so they can reference state slices without prop drilling)
  // ═════════════════════════════════════════════════════════════════════════
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className={`h-2.5 rounded-full overflow-hidden ${ui.th.track}`}>
        ...
        <div className="grid gap-4 grid-cols-1 md:grid-cols-[1fr_1.7fr]">
          ...
          {/* Chapters — click = add to tasks only */}
        ...
        {/* Task list — checkbox is the ONLY completion trigger */}
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .16 }} className="st-card glass rounded-2xl border p-5">
          <div className="flex justify-between items-center mb-4">
            ...
            {taskS.tasks.length > 0 && (
              <span className="font-rajdhani font-bold text-sm" style={{ color: taskS.doneCount === taskS.tasks.length ? accent : (ui.th.light ? "#94a3b8" : "#52525b") }}>{taskPct}%</span>
            )}
          </div>
          {/* TaskInput at module scope — typing never reaches App */}
          <div className="mb-3">
          ...
          <LayoutGroup>
            <div className="flex flex-col gap-2">
              <AnimatePresence initial={false}>
                {taskS.tasks.length === 0 && (
                  <motion.p key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={`text-center py-5 text-sm ${ui.th.muted}`}>
                ...
                {taskS.tasks.map(t => (
                  <motion.div key={t.id} layout="position"
                    ...
                    <motion.button {...TAP} onClick={() => handleToggleTask(t.id)}
                    ...
                    <motion.button {...TAP} onClick={() => taskS.deleteTask(t.id)}
    ...
    return (
      <div className="flex flex-col gap-4">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="st-card glass rounded-2xl border p-5">
          <div className={`flex p-1 rounded-xl mb-6 max-w-xs mx-auto ${ui.th.track}`}>
            {["pomodoro", "stopwatch"].map(m => (
              <motion.button key={m} {...TAP} onClick={() => { audio.playClick(); timer.switchMode(m); }}
          ...
          <div className="flex justify-center mb-6">
            <CircularTimer pct={progress} accent={phaseCol} size={sz}>
              <p className="text-[10px] tracking-[.2em] uppercase mb-1" style={{ color: phaseCol }}>
                {timer.timerMode === "pomodoro" ? (timer.phase === "work" ? "FOCUS" : "BREAK") : "ELAPSED"}
              </p>
              <p className="font-rajdhani font-bold tracking-widest"
                style={{ fontSize: "clamp(36px,10vw,52px)", color: ui.th.light ? "#0f172a" : "#f4f4f5" }}>
                {timer.timerMode === "pomodoro" ? fmt(timer.timerSec) : fmt(timer.swTime)}
              </p>
              {timer.timerMode === "pomodoro" && <p className={`text-[10px] mt-1 ${ui.th.muted}`}>+{XP_POMO} XP on complete</p>}
            </CircularTimer>
          </div>
          ...
          <div className="flex justify-center gap-4 mb-5">
            ...
            <motion.button {...TAP} whileTap={{ scale: .95, rotate: -30 }}
          ...
          {timer.timerMode === "pomodoro" && (
            <div className="flex gap-8 justify-center flex-wrap">
              {[
                { label: "Focus", val: timer.pomoDur, set: v => { timer.setPomoDur(v); if (!timer.running) timer.timerSec; } },
                ...
                <div key={label} className="text-center">
                  ...
                  <div className="flex items-center gap-2">
                    {[{ s: "−", fn: () => set(v => Math.max(1, v - 5)) }, null, { s: "+", fn: () => set(v => Math.min(90, v + 5)) }].map((b, i) =>
                      b ? (
                        <motion.button key={b.s} {...TAP} onClick={() => { audio.playClick(); b.fn(); }}
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .1 }} className="st-card glass rounded-2xl border p-5">
          <p className={`text-[10px] tracking-[.2em] uppercase mb-3 ${ui.th.muted}`}>🎯 CURRENTLY STUDYING</p>
          <select value={timer.topic} onChange={e => timer.setTopic(e.target.value)}
            ...
            {Object.entries(syl.subs).map(([sub, chs]) => (
              <optgroup key={sub} label={sub} style={{ background: ui.th.optBg }}>
                {chs.map(ch => <option key={ch} value={`${sub} — ${ch}`} style={{ background: ui.th.optBg }}>{ch}</option>)}
              </optgroup>
            ))}
          </select>
        </motion.div>
            ...
            <div className="flex flex-col gap-2">
              {game.sessions.slice(-5).reverse().map((s, i) => (
                <div key={i} className="flex justify-between items-center rounded-xl px-3 py-2.5"
                  ...
                  <div><p className={`text-xs ${ui.th.text}`}>{s.subject}</p><p className={`text-[10px] font-mono ${ui.th.muted}`}>{s.time}</p></div>
                  <div className="text-right"><p className="font-rajdhani font-bold text-sm" style={{ color: accent }}>{s.dur}m</p><p className={`text-[10px] ${ui.th.muted}`}>+{XP_POMO}XP</p></div>
                </div>
              ))}
            </div>
          </motion.div>
    ...
    return (
      <div className="flex flex-col gap-4">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}
          ...
          <p className="font-rajdhani font-bold" style={{ fontSize: "clamp(38px,10vw,50px)", color: accent }}>{game.level}</p>
          <p className={`text-sm mb-4 ${ui.th.muted}`}>Current Level</p>
          <div className={`h-2 rounded-full overflow-hidden max-w-xs mx-auto ${ui.th.track}`}>
          ...
          <p className={`text-xs mt-2 font-mono ${ui.th.muted}`}>{game.xp} / {XP_LV * game.level} XP → Level {game.level + 1}</p>
        </motion.div>
  ...
  return (
    <TimerCtx.Provider value={timerCtxVal}>
      {/*
        transition-all duration-500 on the root wrapper handles smooth
        background-color and color fades when theme changes.
        The CSS class swap on <html> (done in useUIState) triggers the
        CSS variables which then transition via `transition: background-color 0.5s`
        in index.css.
      */}
      <div className="app-root transition-all duration-500" style={{ minHeight: "100dvh" }}>

        {/* Sidebar — memo, never touches timer state in props */}
        <Sidebar
          th={ui.th} page={ui.page} setPage={ui.setPage}
          open={ui.sidebarOpen} setOpen={ui.setSidebarOpen}
          accent={accent} xp={game.xp} level={game.level}
        ...
        <main className="st-main" style={{ minHeight: "100dvh" }}>
          {/* Header — memo, reads timer only via TimerCtx in MiniTimer */}
          <Header
            th={ui.th} page={ui.page} accent={accent}
            streak={game.streak} xp={game.xp} meta={meta}
            showTheme={ui.showTheme} setShowTheme={ui.setShowTheme}
            themeId={ui.themeId} setThemeId={ui.setThemeId}
            setShowExam={ui.setShowExam}
            switchExam={syl.switchExam} exam={syl.exam}
            muted={mutedDisplay} onToggleMute={handleToggleMute}
            playClick={audio.playClick}
          />

          {ui.showTheme && <div className="fixed inset-0 z-[49]" onClick={() => ui.setShowTheme(false)} />}

          {/* Content — conditional rendering keeps timer alive across page switches */}
        ...
        <AnimatePresence>
          {ui.showExam && (
            <ExamModal exam={syl.exam} onSelect={syl.switchExam} onClose={() => ui.setShowExam(false)} th={ui.th} />
          )}
        </AnimatePresence>

        {game.lvlUp && (
          <LevelUpToast level={game.lvlUp} accent={accent} onDone={() => game.setLvlUp(null)} />
        )}
      </div>
    </TimerCtx.Provider>
  );
